                ...
                for (let reservationRoom of reservationRooms) {
                  ...
                  this.roomService.getRoomById(reservationRoom.roomID).subscribe((responseRoom: Response) => {
                    if (responseRoom && responseRoom.ec == 0) {
                      console.log(responseRoom.dt.roomTypeID + "   " + roomType.roomTypeID);
                      if (responseRoom.dt.roomTypeID == roomType.roomTypeID) {
                        number++
                        this.guestRoom.push({ name: roomType.name, price: roomType.dailyPrice, number: number, priceTotal: number * roomType.dailyPrice! * this.data.diffDays })
                      }
                    }
                  })
